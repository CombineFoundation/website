...
export default function EditMOUModal({ mou, onCancel, onSave }: EditMOUModalProps) {
  const [uploadingFields, setUploadingFields] = useState<Record<string, boolean>>({});
  const [error, setError] = useState("");
  const [form, setForm] = useState<MOUFormData>({
    title: mou.title,
    paragraphs: mou.paragraphs.length > 0 ? mou.paragraphs : [""],
    image: mou.image,
    imageAlt: mou.imageAlt,
    pdf: mou.pdf || "",
  });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg mx-4 p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-xl font-semibold text-gray-900">Edit MOU</h2>
